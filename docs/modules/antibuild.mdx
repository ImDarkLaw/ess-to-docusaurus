...
interactions on a server-wide basis. It also features an alert system to notify admins when certain items are used.

:::note
EssentialsX Anti-Build does not include any kind of region protection. To restrict building in specific regions we
...
## Configuration
You can access the EssentialsX Anti-Build configuration by opening `/plugins/Essentials/config.yml`, and scrolling down
...
EssentialsX Anti-Build will warn players when they are not allowed to build. Set to false to disable.
...
These are the configurations that allow you to list types of blocks to be blocked.
